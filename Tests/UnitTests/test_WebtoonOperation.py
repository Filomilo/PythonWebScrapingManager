...
def test_RetriveWebcomicInfoByUrlEpicV():
    ...
    assert infoWebtoon.webtoonComicInfo.views=='578,111'
    assert infoWebtoon.webtoonComicInfo.subscriptions=='6,311'
    assert infoWebtoon.webtoonComicInfo.grade==5.83
    assert infoWebtoon.webtoonPageEntries==[
 WebtoonDataTypes.WebtoonPageEntry(thumbnailUrl='', title='Ep. 27 (Last Episode)', date=datetime(2015, 3, 3, 0, 0), likes=918, tx=27),
 WebtoonDataTypes.WebtoonPageEntry(thumbnailUrl='', title='Ep. 26', date=datetime(2015, 2, 26, 0, 0), likes=753, tx=26),
 WebtoonDataTypes.WebtoonPageEntry(thumbnailUrl='', title='Ep. 25', date=datetime(2015, 2, 24, 0, 0), likes=641, tx=25),
 WebtoonDataTypes.WebtoonPageEntry(thumbnailUrl='', title='Ep. 24', date=datetime(2015, 2, 19, 0, 0), likes=780, tx=24),
 WebtoonDataTypes.WebtoonPageEntry(thumbnailUrl='', title='Ep. 23', date=datetime(2015, 2, 17, 0, 0), likes=625, tx=23),
 WebtoonDataTypes.WebtoonPageEntry(thumbnailUrl='', title='Ep. 22', date=datetime(2015, 2, 12, 0, 0), likes=709, tx=22),
 WebtoonDataTypes.WebtoonPageEntry(thumbnailUrl='', title='Ep. 21', date=datetime(2015, 2, 10, 0, 0), likes=750, tx=21),
 WebtoonDataTypes.WebtoonPageEntry(thumbnailUrl='', title='Ep. 20', date=datetime(2015, 2, 5, 0, 0), likes=700, tx=20),
 WebtoonDataTypes.WebtoonPageEntry(thumbnailUrl='', title='Ep. 19', date=datetime(2015, 2, 3, 0, 0), likes=808, tx=19),
 WebtoonDataTypes.WebtoonPageEntry(thumbnailUrl='', title='Ep. 18', date=datetime(2015, 1, 29, 0, 0), likes=731, tx=18)
    ]
...
def test_RetriveWebcomicInfoByUrlWayward():
    ...
    assert infoWebtoon.webtoonComicInfo.genre==["Fantasy","Comedy"]
